...
#define Y_CLOCK RPI_V2_GPIO_P1_11
#define Y_DIR RPI_V2_GPIO_P1_12
#define X_CLOCK RPI_V2_GPIO_P1_13
#define X_DIR RPI_V2_GPIO_P1_15

#define POWER_CTL_CLOCK RPI_V2_GPIO_P1_18
...
void doLaserJob(void *arg)
{
    ...
    for(i=0;i<num_records;i+=4){
        ...
        //4 bit/16 levels power control with DAC
        pw1 = (power & power_gpio_mask) >> 3;
        pw2 = (power & power_gpio_mask >> 1) >> 2;
        ...
        //laser power
        if(power != currentlaserpower){
            bcm2835_gpio_write(POWER_1, power_ctl[pw1]);
            bcm2835_gpio_write(POWER_2, power_ctl[pw2]);
            bcm2835_gpio_write(POWER_3, power_ctl[pw3]);
            bcm2835_gpio_write(POWER_4, power_ctl[pw4]);
        ...
        if (ystep != 1) {
            bcm2835_gpio_write(Y_CLOCK, LOW);
        }
...
int main(int argc, char* argv[])
{
    ...
    bcm2835_gpio_fsel(Y_CLOCK, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(Y_DIR, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(X_CLOCK, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(X_DIR, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(LASER, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(POWER_1, BCM2835_GPIO_FSEL_OUTP);
    bcm2835_gpio_fsel(POWER_2, BCM2835_GPIO_FSEL_OUTP);
